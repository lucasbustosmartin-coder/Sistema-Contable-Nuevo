...
export default function Dashboard({ user, setCurrentView }) {
  ...
  // Calcular las fechas para el rango de un mes antes de hoy
            ...
            <>
              ...
              {detalleFecha && (
                <div className="bg-white rounded-xl shadow-md p-6 mt-6 border-t-4 border-indigo-500 animate-fade-in">
                  ...
                  {/* Totales con el mismo estilo del panel de resumen */}
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                    {Object.entries(detalleFecha.totales).map(([tipo, total], index) => {
                      const colorClassArs = tipo === 'Patrimonio Neto' ? 'text-blue-700' : tipo === 'Pasivo' ? 'text-red-700' : 'text-gray-800';
                      const colorClassUsd = tipo === 'Patrimonio Neto' ? 'text-blue-600' : tipo === 'Pasivo' ? 'text-red-600' : 'text-gray-600';
                      return (
                  ...
                  {/* Detalle por Concepto en formato de tabla (única) */}
