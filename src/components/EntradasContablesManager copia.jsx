...
export default function EntradasContablesManager({ user, setCurrentView }) {
  ...
  useEffect(() => {
    if (view === 'edit') {
      // ✅ Intentar recuperar desde sessionStorage antes de cargar de Supabase
  ...
  const loadEntriesForEditing = async () => {
    ...
    try {
      ...
      // ✅ Guardar en sessionStorage al cargar
      try {
  ...
  const handleDeleteDay = async () => {
    ...
    try {
      ...
      // ✅ Limpiar sessionStorage si se elimina el día
      try {
        sessionStorage.removeItem('editing_day_' + selectedDate);
      } catch (e) {
        console.warn('No se pudo limpiar sessionStorage');
  ...
  const renderEditView = () => {
    // ✅ Calcular totales por tipo en ARS y USD con protección
    ...
    // Estimación de Patrimonio Neto en USD (usa la tasa del primer registro o 1 si no hay)
    const tasaRef = entriesForEditing[0]?.tipoCambio || 1;
    const patrimonioNetoUSD = patrimonioNetoARS / tasaRef;
    ...
    return (
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        {/* Resumen de totales por rubro */}
        <div className="mb-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
          <h4 className="text-sm font-semibold text-gray-700 mb-2">Totales por Rubro</h4>
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr className="text-xs font-medium text-gray-500 uppercase">
                  <th className="px-3 py-2 text-left">Rubro</th>
                  <th className="px-3 py-2 text-right">Valor ARS</th>
              ...
              <tbody className="text-sm">
                {Object.keys(totales).length > 0 ? (
                  <>
                    {totales['Activo Corriente'] && (
                      <tr className="border-t border-gray-200">
                        <td className="px-3 py-2 text-green-700 font-medium">Activo Corriente</td>
                        <td className="px-3 py-2 text-right font-medium text-green-700">
                          ${activoCorriente.toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                        </td>
                        <td className="px-3 py-2 text-right font-medium text-green-700">
                    ...
                    {totales['Activo No Corriente'] && (
                      <tr className="border-t border-gray-200">
                        <td className="px-3 py-2 text-green-500 font-medium">Activo No Corriente</td>
                        <td className="px-3 py-2 text-right font-medium text-green-500">
                          ${(activoNoCorriente).toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                        </td>
                        <td className="px-3 py-2 text-right font-medium text-green-500">
                    ...
                    {totales['Pasivo'] && (
                      <tr className="border-t border-gray-200">
                        <td className="px-3 py-2 text-red-600 font-medium">Pasivo</td>
                        <td className="px-3 py-2 text-right font-medium text-red-600">
                          ${pasivo.toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                        </td>
                        <td className="px-3 py-2 text-right font-medium text-red-600">
                    ...
                    <tr className="border-t-2 border-gray-300 font-bold">
                      <td className="px-3 py-2 text-gray-800">Patrimonio Neto</td>
