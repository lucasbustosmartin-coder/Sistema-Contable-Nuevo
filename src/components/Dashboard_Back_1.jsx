  ...
  const cargarDetalle = async (fecha) => {
    ...
    try {
      ...
      const detallePorTipoYRubro = entriesData.reduce((acc, entry) => {
        ...
        if (!acc[tipoEntry][rubroNombre]) {
          acc[tipoEntry][rubroNombre] = { totalArs: 0, totalUsd: 0, conceptos: [] };
        }

        acc[tipoEntry][rubroNombre].totalArs += entry.importe_ars;
        acc[tipoEntry][rubroNombre].totalUsd += entry.importe_usd;
        acc[tipoEntry][rubroNombre].conceptos.push({
      ...
      setDetalleFecha({ fecha, detalle: detallePorTipoYRubro });
    } catch (err) {
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6">
          {loading ? (
            ...
            <>
              ...
              {/* Detalle por Fecha */}
              {detalleFecha && (
                <div className="bg-white rounded-xl shadow-md p-6 mt-6 border-t-4 border-indigo-500 animate-fade-in">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    {Object.entries(detalleFecha.detalle).map(([tipo, rubros]) => (
                      <div key={tipo} className="col-span-full border rounded-lg p-4">
                        <h4 className="text-lg font-semibold text-gray-800 mb-2">{tipo}</h4>
                        <div className="space-y-4">
                          {Object.entries(rubros).map(([rubro, datos]) => (
                            <div key={rubro} className="bg-gray-50 rounded-lg p-4">
                              <h5 className="font-medium text-gray-700 truncate">{rubro}</h5>
                              <div className="mt-2 flex justify-between text-sm text-gray-600">
                                <span>Total ARS:</span>
                                <span>${datos.totalArs.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                              </div>
                              <div className="flex justify-between text-sm text-gray-600">
                                <span>Total USD:</span>
                                <span>${datos.totalUsd.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                  ...
                  <div className="border-t pt-6">
                    ...
                    {Object.entries(detalleFecha.detalle).map(([tipo, rubros]) => (
                      <div key={tipo} className="mb-6">
                        <h5 className="font-medium text-gray-700 border-b pb-2 mb-3">{tipo}</h5>
                        {Object.entries(rubros).map(([rubro, datos]) => (
                          <div key={rubro} className="mb-4">
                            <h6 className="text-sm font-semibold text-gray-600">{rubro}</h6>
                            <div className="space-y-2 mt-2">
                              {datos.conceptos.map((concepto, index) => (
                                <div key={index} className="flex justify-between text-sm bg-gray-50 p-2 rounded">
                                  <span className="text-gray-800">{concepto.nombre}</span>
                                  <span className="font-medium">
                                    ${concepto.ars.toLocaleString('es-AR', { minimumFractionDigits: 2 })} ARS / ${concepto.usd.toLocaleString('es-AR', { minimumFractionDigits: 2 })} USD
                                  </span>
                                </div>
                              ))}
                            </div>
