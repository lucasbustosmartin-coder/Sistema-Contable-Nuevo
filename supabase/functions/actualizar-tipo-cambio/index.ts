...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const operations = priceData.map((item) => {
      return async () => {
        const fecha = item.date.split('T')[0];
        const tasa = parseFloat(item.usd_mep);

        if (isNaN(tasa) || tasa <= 0) {
          return { error: `Valor de tasa no válido para la fecha ${fecha}` };
        }
        ...
        if (fetchError && fetchError.code !== 'PGRST116') {
          return { error: `Error al buscar registro para la fecha ${fecha}: ${fetchError.message}` };
        }
        ...
        if (existingEntry) {
          ...
          if (updateError) {
            return { error: `Error al actualizar registro para la fecha ${fecha}: ${updateError.message}` };
          }
          ...
          if (insertError) {
            return { error: `Error al insertar registro para la fecha ${fecha}: ${insertError.message}` };
          }
    ...
    return new Response(JSON.stringify({ error: err.message || 'Error desconocido al ejecutar la función.' }), {
      headers: { 'Content-Type': 'application/json', ...corsHeaders },
      status: 500,
    });
  }
