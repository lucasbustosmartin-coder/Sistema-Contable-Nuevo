  ...
  try {
    ...
    const operations = priceData.map((item)=>{
      return async ()=>{
        ...
        const { data: existingEntry, error: fetchError } = await supabase.from('tipos_cambio').select('id').eq('usuario_id', user_id).eq('fecha', fecha).single();
        ...
        if (existingEntry) {
          const { error: updateError } = await supabase.from('tipos_cambio').update({
            tasa: tasa
